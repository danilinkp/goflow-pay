...
func TestBankOperationRepo_Save_And_GetByIdempotencyKey(t *testing.T) {
	...
	op, err := entities.NewBankOperation(acc.AccountId(), bankAcc.BankAccountId(), "withdrawal", "pending", 1000, key, "")
...
func TestBankOperationRepo_UpdateStatusAndExternalID(t *testing.T) {
	...
	op, _ := entities.NewBankOperation(acc.AccountId(), bankAcc.BankAccountId(), "deposit", "pending", 500, "key", "")
	_ = repo.Save(ctx, op)
