...
func MigrateAccounts(ctx context.Context, postgresDSN, mongoDSN, mongoDBName string) error {
	...
	if err = migrateAccountsTable(ctx, pg, db); err != nil {
		return err
	}
	if err = migrateBankAccountsTable(ctx, pg, db); err != nil {
		return err
	}
	if err = migrateAccountOperationsTable(ctx, pg, db); err != nil {
		return err
	}
	if err = migrateBankOperationsTable(ctx, pg, db); err != nil {
		return err
	}
	if err = migrateStatementsTable(ctx, pg, db); err != nil {
		return err
	}
...
func migrateAccountsTable(ctx context.Context, pg *pgxpool.Pool, db *mongo.Database) error {
	fmt.Println("  accounts...")

	rows, err := pg.Query(ctx, `
		SELECT account_id, company_id, balance, currency, status, created_at, updated_at
		FROM accounts
	`)
	if err != nil {
	...
	return bulkInsert(ctx, db.Collection("accounts"), docs)
}

func migrateBankAccountsTable(ctx context.Context, pg *pgxpool.Pool, db *mongo.Database) error {
	fmt.Println("  bank_accounts...")

	rows, err := pg.Query(ctx, `
		SELECT bank_account_id, company_id, name, bic, settlement_account, currency, created_at, updated_at
		FROM bank_accounts
	`)
	if err != nil {
		return fmt.Errorf("query bank_accounts: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&bankAccountId, &companyId, &name, &bic, &settlementAccount, &currency, &createdAt, &updatedAt); err != nil {
			return fmt.Errorf("scan bank_account: %w", err)
		}
		docs = append(docs, bson.M{
			"_id":                bankAccountId,
			"company_id":         companyId,
	...
	return bulkInsert(ctx, db.Collection("bank_accounts"), docs)
}

func migrateAccountOperationsTable(ctx context.Context, pg *pgxpool.Pool, db *mongo.Database) error {
	...
	`)
	if err != nil {
		return fmt.Errorf("query account_operations: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&operationId, &accountId, &counterpartyId, &transactionId,
			...
			return fmt.Errorf("scan account_operation: %w", err)
		}
		docs = append(docs, bson.M{
			"_id":              operationId,
	...
	return bulkInsert(ctx, db.Collection("account_operations"), docs)
}

func migrateBankOperationsTable(ctx context.Context, pg *pgxpool.Pool, db *mongo.Database) error {
	...
	`)
	if err != nil {
		return fmt.Errorf("query bank_operations: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&bankOperationId, &accountId, &bankAccountId, &initiatorId,
			...
			return fmt.Errorf("scan bank_operation: %w", err)
		}
		docs = append(docs, bson.M{
			"_id":              bankOperationId,
			"account_id":       accountId,
	...
	return bulkInsert(ctx, db.Collection("bank_operations"), docs)
}

func migrateStatementsTable(ctx context.Context, pg *pgxpool.Pool, db *mongo.Database) error {
	...
	return bulkInsert(ctx, db.Collection("statements"), docs)
}
